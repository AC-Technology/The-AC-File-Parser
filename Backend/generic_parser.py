...
def lambda_handler(event, context):
    try:
        ...
        # Find carrier and set value to carrier
        carrier = carrier_data.columns[1]
    
        # Find date and set value to date
        date = carrier_data.columns[3]
        datetimeStr = date.strftime("%B %Y")
    
        # Set file
        filename = carrier + " - " + datetimeStr
        # print(filename)
    
    
        # Adjust df columns to the correct row
        ...
        try:
            while more_records:
                search_params = {'criteria': 'Carrier:equals:'+str(carrier),
                    'page':page
                }
                # print(search_params)
            ...
            for i in range(len(agency_code_records)):
                ...
                try:
                    ...
                    agency_code_id_map[current_record['Name']]['contact_id'] = current_record['Contact']['id']
                except:
                    pass
        except:
            print("Search function could not find any agency codes or there is an error")
        ...
        YTD_Submits = int(carrier_data["YTD Submits"].iloc[0])
        YTD_Hit_Ratio = format_percentage(carrier_data["YTD Hit Ratio"].iloc[0])
        Prior_YTD_Quotes = int(carrier_data["Prior YTD Quotes"].iloc[0])
        Prior_YTD_Submits  = int(carrier_data["Prior YTD Submits"].iloc[0])
        Quotes_12MM = int(carrier_data["12MM Quotes"].iloc[0])
        Submits_12MM = int(carrier_data["12MM Quotes"].iloc[0])
        Hit_Ratio_12MM = format_percentage(carrier_data["12MM Hit Ratio"].iloc[0])
        req_body = {
                "data": [
                    {
                        "Name": Name,
                        ...
                        "Submits_12MM": Submits_12MM,
                        "Hit_Ratio_12MM": Hit_Ratio_12MM
                    }
                ]
            }
        print(req_body)
        ...
        Incurred_Loss_12MM = int(carrier_data["Incurred Loss 12MM (PL)"].iloc[0])
        ...
        # Check if the columns are empty. If they are empty then do not make a Personal LOB Total Report
        total = 0
        total = Prior_YTD_DWP + YTD_DWP + YTD_DWP_Growth + DWP_12MM + YTD_NB_PIF + NB_DWP_12MM + Incurred_Loss_12MM + Capincurred_Loss_12MM + Earned_Premium_12MM + Loss_Ratio_12MM
        if total != 0:
        ...
        Incurred_Loss_12MM = int(carrier_data["Incurred Loss 12MM (CL)"].iloc[0])
        ...
        # Check if the columns are empty. If they are empty then do not make a Commercial LOB Total Report
        total = 0
        total = Prior_YTD_DWP + YTD_DWP + YTD_DWP_Growth + DWP_12MM + YTD_NB_PIF + NB_DWP_12MM + Incurred_Loss_12MM + Capincurred_Loss_12MM + Earned_Premium_12MM + Loss_Ratio_12MM
        if total != 0:
        ...
        for i, row in carrier_data.iloc[2:].iterrows():
            ...
            name_string = row["Agency Name"] + " - " + carrier + " - " + word_date
            ...
            YTD_Submits = int(row["YTD Submits"])
            YTD_Hit_Ratio = format_percentage(row["YTD Hit Ratio"])
            Prior_YTD_Quotes = int(row["Prior YTD Quotes"])
            Prior_YTD_Submits  = int(row["Prior YTD Submits"])
            Quotes_12MM = int(row["12MM Quotes"])
            Submits_12MM = int(row["12MM Quotes"])
            Hit_Ratio_12MM = format_percentage(row["12MM Hit Ratio"])
            ...
            print(member_consolidated_count)
            # print(req_body)
        
            # PL Section
            Line_of_Business = 'Personal'
            lob_name_string = row["Agency Name"] + " - " + carrier + " - " + word_date + " - " + Line_of_Business
            ...
            Incurred_Loss_12MM = int(row["Incurred Loss 12MM (PL)"])
            Capincurred_Loss_12MM = int(row['Incurred Loss 12MM (PL)'])
            Earned_Premium_12MM = int(row['Earned Premium 12MM (PL)'])
            Loss_Ratio_12MM = format_percentage(row['12MM LR (PL)'])
    
            # Check if the columns are empty. If they are empty then do not make a Personal LOB Report
            total = 0
            total = Prior_YTD_DWP + YTD_DWP + YTD_DWP_Growth + DWP_12MM + YTD_NB_PIF + NB_DWP_12MM + Incurred_Loss_12MM + Capincurred_Loss_12MM + Earned_Premium_12MM + Loss_Ratio_12MM
            if total != 0:
                req_body = {
            ...
            Incurred_Loss_12MM = int(row["Incurred Loss 12MM (CL)"])
            ...
            # Check if the columns are empty. If they are empty then do not make a Commercial LOB Report
            total = 0
            total = Prior_YTD_DWP + YTD_DWP + YTD_DWP_Growth + DWP_12MM + YTD_NB_PIF + NB_DWP_12MM + Incurred_Loss_12MM + Capincurred_Loss_12MM + Earned_Premium_12MM + Loss_Ratio_12MM
            if total != 0:
                req_body = {
        ...
        # print totals
        print("total new agency codes created:",new_agency_code_count)
        print("total member records created:",member_consolidated_count)
        ...
        data = {"location": "Generic(Excel) Parser Function"}
        client = boto3.client('lambda',region_name='us-east-1')
        response = client.invoke(
