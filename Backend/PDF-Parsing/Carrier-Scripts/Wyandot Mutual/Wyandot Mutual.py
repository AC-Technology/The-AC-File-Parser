...
def main():
    ...
    df = tabula.read_pdf(key, area = [105,30,440,620])
    # print(df)

    Name = "Wyandot Mutual (YE Results) - " + word_date
    Earned_Premium_YTD = currency_format(df[0]["Agency #"][2])
    Incurred_Loss_YTD = currency_format(df[0]["Agency Name"][2])
    YTD_Loss_Ratio = currency_format(df[0]["Unnamed: 0"][2])
    Prior_YTD_DWP = currency_format(df[0]["Agency #"][6])
    YTD_DWP = currency_format(df[0]["Agency Name"][6])
    Prior_YTD_PIF = currency_format(df[0]["Unnamed: 1"][6])
    YTD_PIF = currency_format(df[0]["Unnamed: 2"][6])
    YTD_NB_DWP = currency_format(df[0]["Agency #"][9])
