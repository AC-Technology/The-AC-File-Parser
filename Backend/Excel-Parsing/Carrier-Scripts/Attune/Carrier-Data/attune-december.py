...
# GLOBAL VARIABLES
# Carrier Report ID
# This will come from the 'Attune - December 2021' AC Carrier Report URL
carrier_report_id = 5187612000002363176
...
def main():
    ...
    # Read Carrier Data
    carrier_data = pd.read_excel('Attune\Carrier-Data\Attune December.xlsx')

    carrier_data['New'] = carrier_data['New'].fillna('')
    carrier_data['New.1'] = carrier_data['New.1'].fillna('')
    ...
    for i, row in carrier_data[1:].iterrows():
        ...
        name_string = agency_name + " - Attune - December 2021"

        # This dictionary will hold our API request data when we're ready to send it
        request_body = {}

        YTD_PIF = row['Unnamed: 9']
        YTD_NB_PIF = row['New.1']
        YTD_NB_DWP = row['New']
        if row['New'] != '':
            YTD_NB_DWP = int(row["New"])
        else:
            YTD_NB_DWP = row["New"]
        

        if row['New.1'] != '':
            YTD_NB_PIF = int(row["New.1"])
        else:
            YTD_NB_PIF = row["New.1"]
        # Total Gross Premium Column
        YTD_DWP = is_integer_num(row["Unnamed: 8"])
        print(YTD_DWP)
        try:
