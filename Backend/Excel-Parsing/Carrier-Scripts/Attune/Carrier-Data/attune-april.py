...
# GLOBAL VARIABLES
# Carrier Report ID
# This will come from the 'Attune - April 2022' AC Carrier Report URL
carrier_report_id = 5187612000002219349
...
def main():
    ...
    # Read Carrier Data
    carrier_data = pd.read_excel('Attune\Carrier-Data\Attune Monthly_Production_Report_2022-05-01T1200.xlsx')
    pd.set_option('max_seq_item', None)
    print(carrier_data.columns)
    carrier_data['2022-04.2'] = carrier_data['2022-04.2'].fillna('')
    carrier_data['2022-04.4'] = carrier_data['2022-04.4'].fillna('')
    carrier_data['2022-04.5'] = carrier_data['2022-04.5'].fillna('')
    carrier_data['2022-04'] = carrier_data['2022-04'].fillna('')
    ...
    for i, row in carrier_data[1:].iterrows():
        ...
        name_string = agency_name + " - Attune - April 2022"

        # This dictionary will hold our API request data when we're ready to send it
        request_body = {}

        if row['2022-04.5'] != '':
            YTD_PIF = int(row["2022-04.5"])
        else:
            YTD_PIF = row["2022-04.5"]

        YTD_NB_PIF = (row['2022-04'])
        YTD_NB_DWP = is_integer_num(row['2022-04.2'])

        # Total Gross Premium Column
        YTD_DWP = is_integer_num(row["2022-04.4"])
        print(YTD_DWP)
