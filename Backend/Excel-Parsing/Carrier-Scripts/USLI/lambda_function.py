...
def lambda_handler(event,context):
    ...
    page = 1
    while more_records:
        search_params = {'criteria': 'Carrier:equals:USLI',
            'page':page
        }
    ...
    Name = 'USLI - ' + word_date
    # YTD_DWP = int(carrier_data["TotalGrossComm"].iloc[0])
    # TotalAgentComm = int(carrier_data["TotalAgentComm"].iloc[0])
    # YTD_Submits = int(carrier_data["Received"].iloc[0])
    # YTD_PIF = int(carrier_data["Bound"].iloc[0])
    # Earned_Premium_YTD = int(carrier_data["BndPremium"].iloc[0])
    # YTD_Quotes = int(carrier_data["Quoted"].iloc[0])
    request_body = {
                    "data": [{
                            "Name": Name,
                            "Carrier": "USLI",
    ...
    for i, row in carrier_data[0:].iterrows():
        ...
        agency_code = row["ProducerID_1"]

        name_string = agency_name + ' - ' + Name
        YTD_DWP = int(row['TotalGrossComm'])
        TotalAgentComm = int(row['TotalAgentComm'])
        YTD_Submits = int(row['Received'])
        YTD_PIF = int(row['Bound'])
        Earned_Premium_YTD = int(row['BndPremium'])
        YTD_Quotes = int(row['Quoted'])
    ...
    #     data = {"location": "USLI Parser Function"}
    #     client = boto3.client('lambda')
    #     response = client.invoke(
