...
# Get this ID from the Carrier Report Endpoint
carrier_report_id = 5187612000001638870
...
def main():
    ...
    carrier_data = pd.read_excel('Progressive\Carrier-Data\Progressive - The AC - results through Jan 2022.xlsx', 'By Agent')
    zoho_account_data = pd.read_csv('Zoho-Data\Accounts_001.csv')
    # Fill null values of "Tr 12 LR" Column
    ...
    # agency_code_record_response_count = agency_get_response.json()['info']['count']
    # # print(agency_get_response.json())
    ...
    page = 1
    while more_records:
        search_params = {'criteria': 'Carrier:equals:Progressive', 'page':page
        }
        agency_get_response = requests.get(url=agency_code_search_url,
                                        headers=headers,
                                        params=search_params)
    ...
    for i, row in zoho_account_data.iterrows():
        ...
        account_id_map[account_name] = {'acc_id': str(row['Record Id']).split('_')[1],}
        try:
    ...
    for i, row in carrier_data[12:1817].iterrows():
        ...
        # "Agent" Column: Contains Agency Codes
        agency_code = str(row["Agency Collective"]).strip()
        # print(agency_code)

        # "Agency Name" Column: Contains Names of Agencies
        agency_name = str(row['Unnamed: 1']).strip()
        # print(agency_name)
        #Create Name String 
        name_string = agency_name + " - Progressive - January 2022"
        print(name_string)
        # "Tr 12 WP" Column: Contains Total Rolling 12 Month Written Premium
        DWP_12MM = is_integer_num(row['Unnamed: 6'])
        # print(DWP_12MM)
        # "Tr 12 EP" Column: Total Rolling 12 Month Earned Premium
        Earned_Premium_12MM = is_integer_num(row['Unnamed: 7'])
        # "Tr 12 LR" Column: Contains Total Rolling 12 Month Loss Ratio
        Loss_Ratio_12MM = format_number(row['Unnamed: 8'])
        # print(Loss_Ratio_12MM)
    
        # "Tr 12 IL" Column: Contains Total Rolling 12 Month Incurred Loss
        Incurred_Loss_12MM = is_integer_num(row['Unnamed: 9'])
        # print(type(Incurred_Loss_12MM))

        # "YTD Written Premium" Column: Contains Year to Date Direct Written Premium
        YTD_DWP = is_integer_num(row["Unnamed: 25"])
        # "YTD NBWP" Column: Contains New Business Written Premium 
        YTD_NB_DWP = is_integer_num(row['Unnamed: 26'])
        # "YTD EP" Column: Contains Year to Date Earned Premium
        Earned_Premium_YTD = is_integer_num(row['Unnamed: 27'])
        # "YTD LR" Column: Copntains Year to Date Loss Ratio
        YTD_Loss_Ratio = format_number(row['Unnamed: 28'])
        # print(YTD_Loss_Ratio)
        # "YTD IL" Column: Contains Year to Date Incurred Loss
        Incurred_Loss_YTD = is_integer_num(row['Unnamed: 29'])
        # "WP Prior YTD" Column: Contains Prior Year to Date Written Premium
