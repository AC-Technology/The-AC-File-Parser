...
def lambda_handler(event, context):
    ...
    page = 1
    while more_records:
        search_params = {'criteria': 'Carrier:equals:Ohio Mutual',
            'page':page
        }
    ...
    count = 0
    lob_member_data = []
    # Some rows are empty so we will iloc the rows we will use

    drop = carrier_data[carrier_data["Agency Name"].isna()].index
    ...
    Name = 'Ohio Mutualtest - ' + word_date
    YTD_DWP = carrier_data['DWP'].iloc[-1]
    Earned_Premium_YTD = carrier_data['EP'].iloc[-1]
    Incurred_Loss_YTD = carrier_data['Incurred Losses'].iloc[-1]
    YTD_Loss_Ratio = format_number(carrier_data['Loss Ratio'].iloc[-1])  
    YTD_NB_DWP = carrier_data['New Business'].iloc[-1]
    request_body = {
                "data": [{
                    "Name": Name,
    ...
    for i, row in carrier_data.iterrows():
        ...
        agency_code = str(row["Agency Number"]).strip().replace('.0','')
        name_string = agency_name + " - Ohio Mutual - " + word_date

        # this dictionary will hold our API request data when we're ready to send it
        request_body = {}
        YTD_DWP = row['DWP']
        Earned_Premium_YTD = row['EP']
        Incurred_Loss_YTD = row['Incurred Losses']
        YTD_Loss_Ratio = format_number(row['Loss Ratio'])  
        YTD_NB_DWP = row['New Business']
