...
def main():
    ...
    carrier_data = pd.read_excel(
        'MSA - AC - Agency YTD by Locations february 2022.xlsx')
    ...
    carrier_report_id = 5187612000001628316
    ...
    for i, row in carrier_data.loc[21:73].iterrows():
        count+=1
        print(count)
        ...
        name_string = agency_name + " - Main Street America - February 2022"
        ...
        if type(row["Unnamed: 8"]) != str:
            YTD_DWP_Growth = format_percentage(row["Unnamed: 8"])
        else:
            YTD_DWP_Growth = 0
        ...
        if type(row["Unnamed: 24"]) != str:
            YTD_Loss_Ratio = format_percentage(row["Unnamed: 24"])
        else:
            YTD_Loss_Ratio = 0
        try:
