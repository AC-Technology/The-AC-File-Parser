...
def lambda_handler(event, context):
    ...
    carrier_data = pd.read_excel('s3://excel-parser-files/' + key, engine='openpyxl', sheet_name="Agent Summary")
    ...
    Name = 'Main Street America - ' + word_date
    YTD_DWP = carrier_data['Unnamed: 5'].iloc[18]
    YTD_NB_DWP_Growth = format_number(carrier_data['Unnamed: 3'].iloc[18])
    YTD_DWP_Growth = format_number(carrier_data['Unnamed: 7'].iloc[18])
    DWP_12MM = is_integer_num(carrier_data['Unnamed: 9'].iloc[18])
    YTD_PIF = carrier_data['Agency YTD by Locations'].iloc[18]
    Earned_Premium_YTD = carrier_data['Unnamed: 19'].iloc[18]
    Incurred_Loss_YTD = is_integer_num(carrier_data['Unnamed: 21'].iloc[18])
    request_body = {
                "data": [{
                    "Name": Name,
    ...
    YTD_DWP = carrier_data['Unnamed: 5'].iloc[17]
    YTD_NB_DWP_Growth = format_number(carrier_data['Unnamed: 3'].iloc[17])
    YTD_DWP_Growth = format_number(carrier_data['Unnamed: 7'].iloc[17])
    DWP_12MM = is_integer_num(carrier_data['Unnamed: 9'].iloc[17])
    YTD_PIF = carrier_data['Agency YTD by Locations'].iloc[17]
    Earned_Premium_YTD = carrier_data['Unnamed: 19'].iloc[17]
    Incurred_Loss_YTD = is_integer_num(carrier_data['Unnamed: 21'].iloc[17])
    request_body = {
            "data": [
                {
                    'Carrier': "Main Street America",
    ...
    lob_name_string =  Name + ' - ' + Line_of_Business
    YTD_DWP = carrier_data['Unnamed: 5'].iloc[13]
    YTD_NB_DWP_Growth = format_number(carrier_data['Unnamed: 3'].iloc[13])
    YTD_DWP_Growth = format_number(carrier_data['Unnamed: 7'].iloc[13])
    DWP_12MM = is_integer_num(carrier_data['Unnamed: 9'].iloc[13])
    YTD_PIF = carrier_data['Agency YTD by Locations'].iloc[13]
    Earned_Premium_YTD = carrier_data['Unnamed: 19'].iloc[13]
    Incurred_Loss_YTD = is_integer_num(carrier_data['Unnamed: 21'].iloc[13])
    request_body = {
            "data": [
                {
                    'Carrier': "Main Street America",
    ...
    for i, row in carrier_data[21:-1].iterrows():
        ...
        name_string = agency_name + " - Main Street America - " + word_date

        # This dictionary will hold our API request data when we're ready to send it
        request_body = {}
        # Declare Variables to be sent
        YTD_DWP = row['Unnamed: 6']
        YTD_NB_DWP_Growth = format_number(row['Unnamed: 4'])
        YTD_DWP_Growth = format_number(row['Unnamed: 8'])
        DWP_12MM = is_integer_num(row['Unnamed: 10'])
        YTD_PIF = row['Unnamed: 14']
        Earned_Premium_YTD = row['Unnamed: 20']
        Incurred_Loss_YTD = is_integer_num(row['Unnamed: 22'])

        try:
            # The first thing we try is to use the agency code on this row of carrier data to find the associated account, contact and agency id. 
            # If this agency code is not in the zoho agency code data, there will be an KeyError, handled by the exception part of this statement. 
            # If there is an agency code already in the system, the information is stored into the corresponding variables.
