...
def lambda_handler(event,context):
    ...
    carrier_data = pd.read_excel('s3://excel-parser-files/' + key, engine='openpyxl',sheet_name = "Detail")
    ...
    page = 1
    while more_records:
        search_params = {'criteria': 'Carrier:equals:Kemper',
            'page':page
        }
    ...
    carrier_data = carrier_data.drop(['Unnamed: 0', 'Unnamed: 1',  'Unnamed: 2',  'Unnamed: 3',  'Unnamed: 4',  'Unnamed: 5', 'Unnamed: 6',  'Unnamed: 7'], axis = 1)
    carrier_data.columns = carrier_data.iloc[4]
    carrier_data = carrier_data.drop(carrier_data.index[0:5])
    YTD_DWP = carrier_data['WP '].iloc[-1]
    Incurred_Loss_YTD = carrier_data['IL '].iloc[-1]
    YTD_Quotes = carrier_data['QUOTES'].iloc[-1]
    Earned_Premium_YTD = carrier_data["EP "].iloc[-1]
    YTD_Loss_Ratio = format_number(carrier_data['LR%'].iloc[-1])
    drop = carrier_data[carrier_data["AGENCY_NAME"].isna()].index
    carrier_data = carrier_data.drop(drop)
    ...
    for i, row in carrier_data.iterrows():
        ...
        agency_name = str(row['AGENCY_NAME']).strip()

        agency_code = str(row["AGENCY_CODE"]).strip()
        name_string = agency_name + " - Kemper - " + word_date

        # This dictionary will hold our API request data when we're ready to send it
        request_body = {}

        Incurred_Loss_YTD = row['IL ']
        YTD_Quotes = row['QUOTES']
        Earned_Premium_YTD = row['EP ']
        YTD_DWP = row['WP ']
        YTD_Loss_Ratio = format_number(row['LR%'])
