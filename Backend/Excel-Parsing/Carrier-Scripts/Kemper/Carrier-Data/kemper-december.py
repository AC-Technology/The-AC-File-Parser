...
# GLOBAL VARIABLES
# Carrier Report ID
# This will come from the 'Kemper - December 2021' AC Carrier Report URL
carrier_report_id = 5187612000002363026
...
def main():
    ...
    # Read Kemper December 2021
    carrier_data = pd.read_excel('Kemper\Carrier-Data\Kemper December.xlsx', sheet_name='Member Breakdown')
    ...
    page = 1
    while more_records:
        search_params = {'criteria': 'Carrier:equals:Kemper', 'page':page
        }
    ...
    for i, row in carrier_data[9:25].iterrows():
        ...
        agency_name = str(row['Unnamed: 2']).strip()
        print(agency_name)

        agency_code = str(row['Unnamed: 1']).strip()
        name_string = agency_name + " - Kemper - December 2021"

        # this dictionary will hold our API request data when we're ready to send it
        request_body = {}

        YTD_DWP = row['Unnamed: 5']
        if row['Unnamed: 7'] != '                            -  ':
            Prior_YTD_DWP = int(row["Unnamed: 7"])
        else:
            Prior_YTD_DWP = str(row["Unnamed: 7"]).replace('                            -  ','')
            print(Prior_YTD_DWP)


        
        Earned_Premium_YTD = row['Unnamed: 9']
        Incurred_Loss_YTD = row['Unnamed: 10']
        if row['Unnamed: 7'] != '                            -  ':
            Incurred_Loss_YTD = int(row["Unnamed: 10"])
        else:
            Incurred_Loss_YTD = str(row["Unnamed: 10"]).replace('                       -  ','')
            print(Incurred_Loss_YTD)
        YTD_Loss_Ratio = format_number(row['Unnamed: 12'])
        print(YTD_Loss_Ratio)
        print(Earned_Premium_YTD)
        print(YTD_DWP)
        ...
        try:
            ...
            # Declare Agency Code ID
            agency_code_id = agency_code_response.json()['data'][0]['details']['id']
            new_agency_code_count += 1
            
            # Request Body for API Call to post a record to the Member Consolidated module
            request_body = {
                "data": [{
                    "Name": name_string,
            ...
            # API call to the CRM API to post a record in the Member Consolidated Module.
            consolidated_response = requests.post(
                url=consolidated_url,
                headers=headers,
                data=json.dumps(request_body).encode('utf-8'))

            # If the Consolidated API call returns an error 
